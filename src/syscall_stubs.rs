...
use solana_sdk::program_error::ILLEGAL_OWNER;
...
impl program_stubs::SyscallStubs for TridentSyscallStubs {
    fn sol_log(&self, message: &str) {
        println!("[syscall] sol_log CALLED with message: {}", message);
...
fn sol_get_rent_sysvar(&self, var_addr: *mut u8) -> u64 {
    eprintln!("[TRIDENT] sol_get_rent_sysvar ACTUALLY CALLED");
    let rent_result = get_invoke_context().get_sysvar_cache().get_rent();
    eprintln!("[TRIDENT] Rent cache result: {:?}", rent_result.is_ok());
    get_sysvar(rent_result, var_addr)
}

    fn sol_get_clock_sysvar(&self, var_addr: *mut u8) -> u64 {
        let ctx = get_invoke_context();
        eprintln!("[syscall] InvokeContext ptr: {:p}", ctx as *const _);
        eprintln!("[syscall] SysvarCache ptr: {:p}", ctx.get_sysvar_cache() as *const _);

        let clock_result = ctx.get_sysvar_cache().get_clock();
        eprintln!("[syscall] get_clock() returned: {:?}", clock_result.is_ok());

        get_sysvar(clock_result, var_addr)
    }
    ...
    fn sol_invoke_signed(
        ...
        println!("[syscall] sol_invoke_signed called FIND IT");
...
fn get_sysvar<T: Default + Sysvar + Sized + serde::de::DeserializeOwned + Clone>(
    ...
    println!("[syscall] get_sysvar CALLED");
    match sysvar {
        ...
        Err(_) => {
            eprintln!("[syscall:get_sysvar] -> UNSUPPORTED_SYSVAR");
            ILLEGAL_OWNER//UNSUPPORTED_SYSVAR
        }
    }
}
